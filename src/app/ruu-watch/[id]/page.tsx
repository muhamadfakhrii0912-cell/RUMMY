    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="border-b border-white/10 bg-gradient-to-b from-primary/5 to-background">
        <div className="max-w-5xl mx-auto px-6 pt-28 pb-12">
          <Link href="/ruu-watch" className="text-sm text-primary hover:underline mb-6 inline-block">
            ← Kembali ke RUU Watch
          </Link>
            ...
            <section>
              ...
              <div className="space-y-3">
                {pasalLines.map((pasal, i) => {
                  ...
                  return (
                    <div key={i} className="bg-white/[0.03] border border-white/10 rounded-xl p-4 hover:border-yellow-500/30 transition-colors">
                      <h4 className="text-sm font-bold text-yellow-400 mb-1">{title}</h4>
            ...
            {ruu.promises && ruu.promises.length > 0 ? (
              <div className="space-y-4">
                {ruu.promises.map((relation) => {
                  ...
                  return (
                    <div key={relation.id} className={`bg-white/[0.03] border rounded-xl p-5 ${isContradiction ? "border-red-500/30" : "border-green-500/30"}`}>
                      ...
                      <h4 className="text-sm font-semibold text-white mb-2 line-clamp-1">{relation.promise.title}</h4>
                      
                      {relation.notes && (
                        <div className="mt-3 p-3 bg-white/5 rounded-lg border border-white/5">
                          <p className="text-xs text-muted-foreground"><strong className="text-white">Analisis AI:</strong> {relation.notes}</p>
                        </div>
                      )}
                      
                      <div className="mt-3 flex items-center gap-2 text-xs text-muted-foreground">
                        <div className="w-5 h-5 rounded-full bg-white/10 flex items-center justify-center font-bold text-white text-[10px] uppercase">
                          {relation.promise.politician.name.charAt(0)}
                        </div>
                        <span className="font-medium text-white">{relation.promise.politician.name}</span>
                        <span>•</span>
              ...
              <div className="bg-white/[0.02] border border-dashed border-white/10 rounded-xl p-6 text-center">
                ...
                <h4 className="text-sm font-medium text-white mb-1">Belum Ada Deteksi Hubungan</h4>
            ...
            <section>
              ...
              <div className="space-y-3">
                {ruu.simulations.map((sim) => {
                  ...
                  return (
                    <div key={sim.id} className="bg-white/[0.03] border border-white/10 rounded-xl p-4">
