...
export default async function PromiseDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="border-b border-white/10 bg-gradient-to-b from-primary/5 to-background">
        <div className="max-w-4xl mx-auto px-6 pt-28 pb-12">
          <Link href="/database" className="text-sm text-primary hover:underline mb-6 inline-flex items-center gap-2">
            <ArrowLeft className="w-4 h-4" /> Kembali ke Katalog Janji
          </Link>
          
          <div className="flex items-center gap-3 mb-6 flex-wrap">
            <span className="px-3 py-1 bg-white/10 text-muted-foreground text-[10px] font-bold rounded-full uppercase tracking-wider">
               {promise.category}
            </span>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
         {/* Politician Info */}
         <Link href={`/politicians/${promise.politician.id}`} className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-2xl p-6 mb-8 hover:bg-white/[0.08] transition-colors group cursor-pointer">
            <div className="w-14 h-14 rounded-full bg-gradient-to-br from-primary/20 to-blue-500/20 border border-white/10 flex items-center justify-center text-xl font-bold text-white shrink-0">
               {promise.politician.name.charAt(0)}
            </div>
            <div className="flex-1">
               <h3 className="text-sm text-muted-foreground mb-1">Janji dari:</h3>
               <div className="text-lg font-bold text-white group-hover:text-primary transition-colors">{promise.politician.name}</div>
               ...
               <div className="space-y-4">
                  {promise.legislations.map(relation => (
                     <Link key={relation.id} href={`/ruu-watch/${relation.legislation.id}`}>
                        <div className="bg-white/5 border border-white/10 rounded-xl p-5 hover:border-primary/50 transition-colors">
                           ...
                           <h4 className="font-bold text-white mb-2">{relation.legislation.title}</h4>
         ...
         <div className="bg-primary/10 border border-primary/20 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6">
            <div>
               <h3 className="font-bold text-white mb-1">Ikut Kawal Janji Ini</h3>
            ...
            {session ? (
               ...
               <Link href="/login" className="bg-white text-background hover:bg-neutral-200 px-6 py-2.5 rounded-lg text-sm font-bold shadow-[0_0_15px_-3px_rgba(255,255,255,0.4)]">
                  Login & Pantau
               </Link>
            )}
         </div>
