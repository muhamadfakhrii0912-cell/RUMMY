...
import { ArrowLeft, Award, Activity, Database, CheckCircle, XCircle, AlertTriangle } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      <div className="border-b border-white/10 bg-gradient-to-b from-primary/5 to-background">
        <div className="max-w-5xl mx-auto px-6 pt-28 pb-12">
          <Link href="/politicians" className="text-sm text-primary hover:underline mb-6 inline-flex items-center gap-2">
            <ArrowLeft className="w-4 h-4" /> Kembali ke Daftar Politisi
          </Link>
          
          <div className="flex flex-col md:flex-row gap-8 items-start md:items-center">
             <div className="w-32 h-32 rounded-3xl bg-gradient-to-br from-primary/20 to-blue-500/20 border-2 border-white/10 flex items-center justify-center text-5xl font-black text-white shadow-2xl shrink-0">
             ...
             {/* Score Card */}
             <div className="shrink-0 bg-white/5 border border-white/10 rounded-2xl p-6 text-center w-full md:w-64">
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
           <div className="bg-white/5 border border-white/10 rounded-xl p-5 text-center">
             ...
             <div className="text-2xl font-bold text-white">{politician.promises.length}</div>
             <div className="text-xs text-muted-foreground">Total Janji</div>
           </div>
           <div className="bg-white/5 border border-white/10 rounded-xl p-5 text-center">
             <CheckCircle className="w-6 h-6 text-green-400 mx-auto mb-2" />
             <div className="text-2xl font-bold text-white">{fulfilled}</div>
         ...
         <h2 className="text-2xl font-bold text-white mb-6">Track Record Janji Politik</h2>
         
         <div className="space-y-4">
           {politician.promises.length === 0 ? (
             <div className="text-center py-12 bg-white/5 border border-white/10 rounded-2xl">
                <AlertTriangle className="w-12 h-12 text-muted-foreground/50 mx-auto mb-4" />
                <h3 className="text-lg font-bold text-white mb-2">Belum ada janji tercatat</h3>
             ...
             politician.promises.map((promise) => (
               <div key={promise.id} className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 transition-all hover:bg-white/[0.06] hover:border-primary/40 flex flex-col md:flex-row gap-6">
                 <div className="flex-1">
